...
func (s *TagService) List(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		return c.Status(fiber.StatusInternalServerError).JSON(models.ErrorResponse("Failed to fetch tags"))
	}

	return c.JSON(models.SuccessResponse(tags, ""))
}
...
func (s *TagService) Create(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		return c.Status(fiber.StatusInternalServerError).JSON(models.ErrorResponse("Failed to create tag"))
	}

	return c.Status(fiber.StatusCreated).JSON(models.SuccessResponse(tag, "Tag created successfully"))
}
...
func (s *TagService) Update(c *fiber.Ctx) error {
	...
	if err != nil {
		...
		return c.Status(fiber.StatusInternalServerError).JSON(models.ErrorResponse("Failed to update tag"))
	}

	return c.JSON(models.SuccessResponse(tag, "Tag updated successfully"))
}
...
func (s *TagService) Delete(c *fiber.Ctx) error {
	...
	if err := s.repo.Delete(c.Context(), id); err != nil {
		...
		return c.Status(fiber.StatusInternalServerError).JSON(models.ErrorResponse("Failed to delete tag"))
	}

	return c.JSON(models.SuccessResponse(nil, "Tag deleted successfully"))
}
